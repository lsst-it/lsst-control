...
describe 'lsstcam-dc9.cp.lsst.org', :sitepp do
  on_supported_os.each do |os, os_facts|
    ...
    context "on #{os}" do
      ...
      context 'with br1511' do
        ...
        it { expect(nm_keyfile['ipv4']['address1']).to eq('139.229.175.109/26,139.229.175.126') }
      ...
      context 'with br1701' do
        ...
        it { expect(nm_keyfile['ipv4']['address1']).to eq('139.229.173.150/27,139.229.173.129') }
        it { expect(nm_keyfile['ipv4']['dns']).to eq('139.229.160.53;139.229.160.54;139.229.160.55;') }
        it { expect(nm_keyfile['ipv4']['dns-search']).to eq('cp.lsst.org;') }
        it { expect(nm_keyfile['ipv4']['method']).to eq('manual') }
