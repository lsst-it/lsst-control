...
describe 'lsstcam-mcm1.cp.lsst.org', :sitepp do
  on_supported_os.each do |os, os_facts|
    ...
    context "on #{os}" do
      ...
      context 'with br1511' do
        ...
        it { expect(nm_keyfile['ipv4']['address1']).to eq('139.229.175.112/26,139.229.175.126') }
      ...
      context 'with br1400' do
        let(:interface) { 'br1400' }

        it_behaves_like 'nm enabled interface'
        it_behaves_like 'nm bridge interface'
        it_behaves_like 'nm manual interface'

        it { expect(nm_keyfile['ipv4']['address1']).to eq('139.229.170.57/24,139.229.170.254') }
        it { expect(nm_keyfile['ipv4']['dns']).to eq('139.229.160.53;139.229.160.54;139.229.160.55;') }
        it { expect(nm_keyfile['ipv4']['dns-search']).to eq('cp.lsst.org;') }
        it { expect(nm_keyfile['ipv4']['method']).to eq('manual') }
