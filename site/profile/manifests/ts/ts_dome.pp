class profile::ts::ts_dome{
	...
	if $labview_version == "2015" {
		exec{ "get_labview":
			...
			onlyif => "test ! -d /tmp/lvruntime2015 && ! -d /etc/natinst/ && -d /usr/local/natinst/"
		}
		...
		exec{ "get_labview":
			...
			notify => Exec["install_labview"],
			onlyif => "test ! -d /tmp/lvruntime2016 && ! -d /etc/natinst/ && -d /usr/local/natinst/"
		}
	}
	...
	exec{"install_labview":
		...
		onlyif => "test ! -f /tmp/lvruntime${$labview_version}/LabVIEW${$labview_version}RTE_Linux.tgz && ! -d /etc/natinst/ && -d /usr/local/natinst/"
	}
