...
class profile::it::graylog {
  ...
  exec{ 'Create SSL certificate':
    ...
    command => "openssl req -x509 -days ${certificate_duration} \
                -nodes -newkey ${ssl_key_algorithm} -config ${ssl_config_dir}/${ssl_config_filename} \
                -keyout ${ssl_config_dir}/${ssl_keyout_filename} -out ${ssl_config_dir}/${ssl_graylog_cert_filename}",
  ...
  exec{ 'Update keytool' :
    ...
    command => "keytool -noprompt -importcert -keystore ${ssl_config_dir}/${graylog_cacert_filename} \
                -storepass ${ssl_graylog_cert_pass} -alias graylog-self-signed \
                -file ${ssl_config_dir}/${ssl_graylog_cert_filename}",
  ...
  class { 'graylog::server':
    ...
    config          => {
      # Fill in your password secret, must have more than 16 characters
      password_secret       => lookup('graylog_server_password_secret'),
      # Fill in your root password hash
