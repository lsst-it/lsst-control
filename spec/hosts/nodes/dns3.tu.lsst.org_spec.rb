...
describe 'dns3.tu.lsst.org', :sitepp do
  on_supported_os.each do |os, facts|
    next if os =~ %r{centos-7-x86_64}

    context "on #{os}" do
      let(:facts) { facts.merge(fqdn: 'dns3.tu.lsst.org') }
      let(:node_params) do
        {
          role: 'dnscache',
          site: 'tu',
      ...
      context 'with ens3' do
        ...
        it { expect(nm_keyfile['ipv4']['address1']).to eq('140.252.146.73/27,140.252.146.65') }
