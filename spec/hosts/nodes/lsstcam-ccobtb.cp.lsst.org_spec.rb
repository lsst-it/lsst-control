  ...
  on_supported_os.each do |os, os_facts|
    ...
    context "on #{os}" do
      ...
      context 'with eno1' do
        ...
        it { expect(nm_keyfile['ipv4']['address1']).to eq('139.229.175.83/26,139.229.175.126') }
      ...
      context 'with enp6s0' do
        ...
        it_behaves_like 'nm ethernet interface'
        it_behaves_like 'nm manual interface'
        it { expect(nm_keyfile['ipv4']['address1']).to eq('192.168.0.200/24') }
      ...
      context 'with enp7s0' do
        ...
        it_behaves_like 'nm ethernet interface'
        it_behaves_like 'nm manual interface'
        it { expect(nm_keyfile['ipv4']['address1']).to eq('192.168.2.1/24') }
        it { expect(nm_keyfile['ipv4']['method']).to eq('manual') }
        it { expect(nm_keyfile['ipv6']['method']).to eq('disabled') }
      end
