...
describe 'lsstcam-shutter01.cp.lsst.org', :sitepp do
    ...
    context "on #{os}" do
      let(:facts) do
        override_facts(os_facts,
                       fqdn: 'lsstcam-shutter01.cp.lsst.org',
                       is_virtual: false,
                       virtual: 'physical',
                       dmi: {
                         'product' => {
      ...
      context 'with enp0s31f6' do
        ...
        it { expect(nm_keyfile['ipv4']['address1']).to eq('139.229.175.84/26,139.229.175.126') }
