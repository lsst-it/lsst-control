...
describe 'lsstcam-quad.cp.lsst.org', :sitepp do
  on_supported_os.each do |os, os_facts|
    ...
    context "on #{os}" do
      ...
      context 'with enp0s31f6' do
        ...
        it { expect(nm_keyfile['ipv4']['address1']).to eq('139.229.175.80/26,139.229.175.126') }
      ...
      context 'with enp3s0' do
        ...
        it_behaves_like 'nm ethernet interface'
        it_behaves_like 'nm manual interface'
        it { expect(nm_keyfile['ipv4']['address1']).to eq('192.168.1.120/24') }
      end
    end # on os
  end # on_supported_os
end
